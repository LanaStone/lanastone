...
1. Изменить конфиг сборки для VPS/PM2
   ...
   - Запускать его через `start.mjs`: внешний шлюз слушает `0.0.0.0:3000`, сразу отвечает healthcheck и проксирует во внутренний Nitro-сервер на `127.0.0.1:3001`.

2. Добавить правильные npm-скрипты
   - `build`: собирает приложение.
   - `start`: запускает `node start.mjs`.
   - Dockerfile должен содержать `ENTRYPOINT ["node", "start.mjs"]`, чтобы Timeweb не зависел от внешнего поля «Команда запуска».
