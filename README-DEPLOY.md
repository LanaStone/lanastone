...
- сервер запускается через `node start.mjs`: внешний шлюз слушает `0.0.0.0:3000`, сразу отвечает на healthcheck Timeweb и проксирует сайт во внутренний Nitro-сервер;
- `Dockerfile` открывает порт `3000`, проверяет `/api/public/health` и содержит `ENTRYPOINT ["node", "start.mjs"]`, поэтому контейнер сам знает, что запускать даже без поля **«Команда запуска»**.
