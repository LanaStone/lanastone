...
- сервер запускается через `node start.mjs`: встроенный Node-шлюз слушает строго `0.0.0.0:3000`, сразу отвечает на healthcheck Timeweb и обслуживает собранный сайт;
- `Dockerfile` открывает порт `3000`, проверяет `/api/public/health` и содержит `CMD ["node", "start.mjs"]`, поэтому контейнер сам знает, что запускать даже без поля **«Команда запуска»**.
