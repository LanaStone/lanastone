...
- `Dockerfile` открывает порт `3000`, задаёт `0.0.0.0:3000`, проверяет `/api/public/health` и содержит `ENTRYPOINT`, поэтому контейнер сам знает, что запускать.

Если в Timeweb есть поле **«Команда запуска»**, оставьте его пустым. Если поле обязательно — укажите:
```bash
sh start.sh
```
